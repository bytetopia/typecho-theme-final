...
class ViewsCounter_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function activate()
    {
        ...
        $db = Typecho_Db::get();
        $prefix = $db->getPrefix();
        // Convert the table name to lowercase for PostgreSQL, which typically uses lowercase table identifiers
        $tableName = strtolower($prefix . 'contents');

        // Check if 'viewsNum' column exists 
        // Since PostgreSQL doesn't have a direct way to check columns from SQL, use the information_schema
        $columnExists = $db->fetchRow(
            $db->select('column_name')
            ->from('information_schema.columns')
            ->where('table_name = ?', $tableName)
            ->where('column_name = ?', 'views') // PostgreSQL is case-insensitive by default
        );

        if (!$columnExists) {
            // Add 'viewsNum' column as an integer with a default value of 0
            $db->query('ALTER TABLE ' . $tableName . ' ADD COLUMN views INTEGER DEFAULT 0;');
        }
